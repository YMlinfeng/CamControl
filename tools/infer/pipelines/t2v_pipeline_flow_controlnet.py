...
@dataclass
class T2VFlowPipelineControlNetConfig(T2VPixArtAlphaPipelineConfig):
    """Configuration for Pipeline instantiation"""

    _target: Type = field(default_factory=lambda: T2VFlowControlNetPipeline)
    ...
    t2v_ratio: float = 1.0
    camclone_ratio:  Optional[float] = 0.5
    ## ======== Add For Ref Video ========= ##
    ref_video_column_name: Optional[str] = 'ref_data'
    content_ref_video_column_name: Optional[str] = 'content_ref_data'
    ## ======== add for controlnet ======== ##
    vae_config_for_conditions: Optional[VisualTokenizerConfig] = None
    """ Control VAE """
    control_transformer_config: Optional[Transformer2DModelConfig] = None
    """ Control Transformer """
    pre_trained_control_transformer_ckpt: Optional[str] = "/m2v_intern/luoyawen/Coding/Kelin/m2v_camclone_v2/exps/0040--1b_camclonemaster_node_12_ablation_controlnet/checkpoints/checkpoint-192000/ema/conditions_transformer.ckpt"
    """ Resume ckpt path of controlnet transformer """

    """ 2. self.config.pipeline.from_pretrained() 进行调度 """
    def from_pretrained(self, **kwargs) -> Any:
        # kwargs一开始是空的
        conditions_processor = None
        conditions_transformer = self.initialize_conditions_processor()
        print("Pipeline starts with No Copy Vae Encoder")
        
        # Load pre_trained_ckpt
        if self.pre_trained_control_transformer_ckpt:
            conditions_processor_ckpt = torch.load(self.pre_trained_control_transformer_ckpt, map_location="cpu")
            conditions_transformer.load_state_dict(conditions_processor_ckpt, strict=True) 
            print(f"Load pretrained conditions_transformer from {self.pre_trained_control_transformer_ckpt}")

        kwargs.update({"conditions_processor": conditions_processor})
        kwargs.update({"conditions_transformer": conditions_transformer})
        kwargs.update({"pre_trained_control_transformer_ckpt": self.pre_trained_control_transformer_ckpt})
        pipeline = super().from_pretrained(**kwargs)
        return pipeline

    """ 2. initialize conditions processor """
    def initialize_conditions_processor(self):
        conditions_transformer = self.control_transformer_config.from_pretrained(self.ckpt_path)
        return conditions_transformer

class T2VFlowControlNetPipeline(T2VPixArtAlphaPipeline, PipelineMixin):
    # 这里把T2VPixArtAlphaPipeline中的__init__覆盖掉
    def __init__(
            self,
            #
            # helper,
            conditions_processor,
            conditions_transformer,
            #
            ...
            self.register_modules(clip_g_model=clip_g_model, clip_l_model=clip_l_model, tokenizer_clip=tokenizer_clip, conditions_processor=conditions_processor, conditions_transformer=conditions_transformer)
            """ 这里 conditions_processor + conditions_transformer 都是要设置在required_parameters里面 """
            
    def forward(self, batch):

        with measure_time("VAE", self.pipeline_config.measure_time):
            latents, attention_mask = self.get_latents(batch)
            ref_latents, ref_attention_mask = self.get_latents_for_conditions_nocopy_vae(batch[self.pipeline_config.ref_video_column_name])  # add for ref video
        ...
        t_expand = self.zero_condition_frame_timestep(t_expand, num_frames, [0])
        timestep = t[:, None, None, None, None]
        ...
        outputs_from_CtrlTransformer = self.conditions_transformer(
            ref_latents, ##latents
        ...
        with measure_time("Transformer", self.pipeline_config.measure_time):
            v = self.transformer(
                ...
                return_dict=False,
                ###
                outputs_from_CtrlTransformer=outputs_from_CtrlTransformer,
                ###
            )[0]
        if attention_mask == None:
            return self.compute_loss({"pred": v[::, ::, 1:20], "target": u[::, ::, 1:20]}, attention_mask)
        else:
            return self.compute_loss({"pred": v[::, ::, 1:20], "target": u[::, ::, 1:20]}, attention_mask[::, ::, 1:20])
    ...
    def __call__(
        ...
        **kwargs,
        ...
        with measure_time("VAE", self.pipeline_config.measure_time):
            ref_latents, _ = self.get_latents_for_conditions_nocopy_vae(batch[self.pipeline_config.ref_video_column_name])  # add for ref video
            content_ref_latents, _ = self.get_latents_for_conditions_nocopy_vae(batch[self.pipeline_config.content_ref_video_column_name])  # add for ref video
            input_condition_latent = content_ref_latents[::, ::, :1]  # get first frame latent
            input_latent_index = [0]
            input_condition_index = [0]
            height, width = batch[self.pipeline_config.ref_video_column_name][0].shape[-2:] # assume condition image is resized in base_data
        ...
        # concat latent and ref latent
        # latents = torch.cat([latents, ref_latents, content_ref_latents], dim=2)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                outputs_from_CtrlTransformer = self.conditions_transformer(
                    torch.cat([ ref_latents] * 2), ##latents
            ...
            latents, _ = self.add_input_condition(latents, input_condition_latent, input_latent_index, input_condition_index, current_timestep)
            latents = latents[::, ::, :20]
