...
@dataclass
class T2VFlowPipelineCamCloneMasterConfig(T2VPixArtAlphaPipelineConfig):
    """Configuration for Pipeline instantiation"""

    _target: Type = field(default_factory=lambda: T2VFlowCamCloneMasterPipeline)
...
class T2VFlowCamCloneMasterPipeline(T2VPixArtAlphaPipeline, PipelineMixin):
    ...
    def forward(self, batch):
        ...
        with measure_time("VAE", self.pipeline_config.measure_time):
            ...
            if attention_mask != None:
                latents, attention_mask = torch.cat([latents, ref_latents, content_ref_latents], dim=2), torch.cat([attention_mask, ref_attention_mask, content_ref_attention_mask], dim=2)  # concat顺序是先latent后ref latent, content ref latent
            else:
