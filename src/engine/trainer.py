...
class TrainerConfig(ExperimentConfig):
    ...
    trainable_only_layers: Optional[str] = None  # add for train only some layers
    """List of layer names need train""" 
...
class Trainer:
        ...
        for module in module_info:
            if isinstance(module, str):
                ...
                if module == 'transformer':
                    if self.config.trainable_only_layers is None: # TODO
                        for name, param_module in model.named_modules():
                            for params in param_module.parameters():
                                params.requires_grad_(requires_grad)
                            print(f"{name} module is trainable.")
                    
                    elif self.config.trainable_only_layers == 'attnt':
                        for name, param_module in model.named_modules():
                            if ("attnt" in name):
                                for params in param_module.parameters():
                                    params.requires_grad_(requires_grad)
                    ...
                    elif self.config.trainable_only_layers == "attnt_projector":
                        for name, param_module in model.named_modules():
                            if "attnt" in name:
                                for params in param_module.parameters():
                                    params.requires_grad_(requires_grad)
                                print(f"{name} module is trainable.")
                            if "cam_projector" in name:
                                for params in param_module.parameters():
                                    params.requires_grad_(requires_grad)
                                print(f"{name} module is trainable.")
                            if "cam_encoder" in name:
                                for params in param_module.parameters():
                                    params.requires_grad_(requires_grad)
                                print(f"{name} module is trainable.")

                        # if self.config.train_only_layers:
                        #     if self.config.train_only_layers == "attnt_normt":
                        #         if ("attnt" in name or "normt" in name):
                        #             for params in param_module.parameters():
                        #                 params.requires_grad_(requires_grad)
                        #             print(f"{name} module is trainable.")
                        #     elif self.config.train_only_layers == "attnt_ref_latent_convert_layer":
                        #         if ("attnt" in name or "ref_latent_convert_layer" in name):
                        #             for params in param_module.parameters():
                        #                 params.requires_grad_(requires_grad)
                        #             print(f"{name} module is trainable.")
                        #     else:
                        #         if (self.config.train_only_layers in name):
                        #             for params in param_module.parameters():
                        #                 params.requires_grad_(requires_grad)
                        #             print(f"{name} module is trainable.")
            else:
    ...
    def save_ckpt(self, global_step):
        ...
        if self.config.use_ema:
            ...
            """ 新增加的模块，需要在这里都增加保存路径，像下面的transformer一样 """
            if hasattr(self.manager, "transformer"):
                if torch.distributed.get_rank() == 0:
                    print(f"begin save ckpt")
                    torch.save(self.manager.transformer.state_dict(), os.path.join(ema_path, "transformer.ckpt"))
                    print(f"saved ckpt!")
            if hasattr(self.manager, "conditions_processor"):
                torch.save(self.manager.conditions_processor.state_dict(), os.path.join(ema_path, "conditions_processor.ckpt"))
            if hasattr(self.manager, "conditions_transformer"):
                torch.save(self.manager.conditions_transformer.state_dict(), os.path.join(ema_path, "conditions_transformer.ckpt"))
