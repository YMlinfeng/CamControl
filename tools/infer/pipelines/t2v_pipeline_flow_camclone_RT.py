...
@dataclass
class T2VFlowCamCloneRTPipelineConfig(T2VPixArtAlphaPipelineConfig):
    """Configuration for Pipeline instantiation"""

    _target: Type = field(default_factory=lambda: T2VFlowCamCloneRTPipeline)
...
class T2VFlowCamCloneRTPipeline(T2VPixArtAlphaPipeline, PipelineMixin):
    ...
    def forward(self, batch):
        ...
        with measure_time("VAE", self.pipeline_config.measure_time):
            ...
            if attention_mask != None:
                latents, attention_mask = torch.cat([latents,  content_ref_latents], dim=2), torch.cat([attention_mask, content_ref_attention_mask], dim=2)  # concat顺序是先latent后ref latent, content ref latent
            else:
                latents = torch.cat([latents, content_ref_latents], dim=2)
        
        batch_size, num_frames = latents.shape[0], latents.shape[2]

        # Add for RT
        pose_embeddings = batch['pose_embeddings']
        indices = torch.linspace(0, 76, 20).long().to(content_ref_latents.device)
        pose_embeddings = [torch.index_select(pose_embedding.to(content_ref_latents.device).to(content_ref_latents.dtype), 1, indices) for pose_embedding in pose_embeddings]
        pose_embeddings = torch.cat(pose_embeddings, dim=0)
        # import pdb; pdb.set_trace()
        plucker_embeddings = batch['plucker_embeddings']
        plucker_embeddings = [torch.index_select(plucker_embedding.to(content_ref_latents.device).to(content_ref_latents.dtype), 1, indices) for plucker_embedding in plucker_embeddings]
        plucker_embeddings = torch.cat(plucker_embeddings, dim=0)
        # print(f"plucker embedding shape is {plucker_embeddings.shape}")  # [2, 20, 48, 84, 6]
        ...
        # else:
        #     # i2v
        condition_index = [0] + list(range(20, 40))
        # condition_index = [0] + list(range(20, 60))
        ...
        with measure_time("Transformer", self.pipeline_config.measure_time):
            v = self.transformer(
                ...
                pose_embeddings = plucker_embeddings,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        with measure_time("VAE", self.pipeline_config.measure_time):
            ...
            ref_condition_latent = content_ref_latents
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                pose_embeddings_input = torch.cat([pose_embeddings] * 2) if do_classifier_free_guidance else pose_embeddings
                if self.pipeline_config.match_snr:
                    scale_factor = latents.shape[2] ** 0.5
                ...
                latent_model_input, current_timestep = self.add_input_condition(
                        latent_model_input, input_condition_latent, input_latent_index, input_condition_index, current_timestep, ref_condition_latent
                )
                ...
                v_pred = self.transformer(
                    ...
                    added_cond_kwargs=added_cond_kwargs,
                    pose_embeddings= pose_embeddings_input,
                    return_dict=False,
                )[0]
