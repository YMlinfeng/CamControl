...
class M2V:
    ...
    def inference(self, batch):
        ...
        video_data = (self.pipeline(prompt, generator=generator, batch=batch, **self.config.pipeline.call, task="recamclone")["images"].cpu().to(torch.float32).numpy() * 255).astype(
            np.uint8
        )  # (B F) C H W
