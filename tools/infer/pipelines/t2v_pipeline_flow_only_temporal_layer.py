...
@dataclass
class T2VFlowPipelineOnlyTemporalConfig(T2VPixArtAlphaPipelineConfig):
    """Configuration for Pipeline instantiation"""

    _target: Type = field(default_factory=lambda: T2VFlowOnlyTemporalPipeline)
...
class T2VFlowOnlyTemporalPipeline(T2VPixArtAlphaPipeline, PipelineMixin):
    ...
    def forward(self, batch):
        ...
        with measure_time("VAE", self.pipeline_config.measure_time):
            ...
            content_ref_latents, content_ref_attention_mask = self.get_latents_for_conditions_nocopy_vae(batch[self.pipeline_config.content_ref_video_column_name])  # add for ref video
            if np.random.rand() > self.pipeline_config.camclone_ratio:
                content_ref_latents = torch.zeros_like(content_ref_latents).to(content_ref_latents.device)
            if attention_mask != None:
                ref_latents, ref_attention_mask = torch.cat([ref_latents, content_ref_latents], dim=2), torch.cat([ref_attention_mask, content_ref_attention_mask], dim=2)  # concat顺序是先latent后ref latent, content ref latent
            else:
                ref_latents, ref_attention_mask = torch.cat([ref_latents, content_ref_latents], dim=2), None
        ...
        ref_t_expand = self.zero_condition_frame_timestep(t[:, None, None, None, None], num_frames*3, [0] + list(range(20, 60)))
        ...
        with measure_time("Transformer", self.pipeline_config.measure_time):
            v = self.transformer(
                ...
                # add for only temporal layer
                ref_latents=ref_latents,
                ref_attention_mask=ref_attention_mask,
                ref_timestep = ref_t_expand * 999,
                return_dict=False,
            )[0]
        if attention_mask == None:
    ...
    def add_input_condition(self, latents, input_conditions, input_latent_index, input_condition_index, timestep=None,
        ...
        #     latents[:, :, 20:] = ref_video_latent

        if timestep is not None:
            if ref_video_latent is not None:
                timestep = timestep.view(-1, 1, 1, 1, 1).repeat_interleave(60, dim=2)  # b -> b 1 f 1 1
            else:
                timestep = timestep.view(-1, 1, 1, 1, 1).repeat_interleave(20, dim=2)  # b -> b 1 f 1 1
    ...
    def __call__(
        ...
        **kwargs,
        ...
        with measure_time("VAE", self.pipeline_config.measure_time):
            ...
            ref_condition_latent = torch.cat([ref_latents, content_ref_latents], dim=2)
        ...
        # print(f"debug: cond_num_frames is {cond_num_frames}")
        added_cond_kwargs["prompt_embeds_pooled"] = prompt_embeds_pooled
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                current_timestep_ori = current_timestep 
                # predict noise model_output

                # current_timestep = current_timestep[:, None, None, None, None]

                # t_expand = self.zero_condition_frame_timestep(current_timestep, num_frames, [0])
                # ref_t_expand = self.zero_condition_frame_timestep(t[:, None, None, None, None], num_frames*3, [0] + list(range(20, 60)))
                ...
                _, current_timestep_ref = self.add_input_condition(
                        latent_model_input, input_condition_latent, input_latent_index, input_condition_index, current_timestep_ori, ref_condition_latent
                )
                # print(f"current_timestep shape is {current_timestep.shape}")
                # print(f"current_timestep_ref shape is {current_timestep_ref.shape}")
                # import pdb; pdb.set_trace()
                # print(f"latent_model_input shape is {latent_model_input.shape}")
                v_pred = self.transformer(
                    ...
                    added_cond_kwargs=added_cond_kwargs,
                    # add for only temporal layer
                    ref_latents= torch.cat([ref_condition_latent] * 2),
                    ref_attention_mask=None,
                    ref_timestep = current_timestep_ref * 999,
                    return_dict=False,
                )[0]
