...
@dataclass
class T2VFlowPipelineChannelConcatConfig(T2VPixArtAlphaPipelineConfig):
    """Configuration for Pipeline instantiation"""

    _target: Type = field(default_factory=lambda: T2VFlowChannelConcatPipeline)
...
class T2VFlowChannelConcatPipeline(T2VPixArtAlphaPipeline, PipelineMixin):
    ...
    def forward(self, batch):
        ...
        with measure_time("VAE", self.pipeline_config.measure_time):
            ...
            latents = torch.cat([latents, ref_latents, content_ref_latents], dim=1)
                # , torch.cat([attention_mask], dim=2)  # concat顺序是先latent后ref latent, content ref latent
            # else:
        ...
        if attention_mask == None:
            return self.compute_loss({"pred": v[::, ::, 1:20], "target": u[::, :8, 1:20]}, attention_mask)
        else:
            return self.compute_loss({"pred": v[::, ::, 1:20], "target": u[::, :8, 1:20]}, attention_mask[::, ::, 1:20])
    ...
    def __call__(
        ...
        **kwargs,
        ...
        with measure_time("VAE", self.pipeline_config.measure_time):
            ...
            ref_condition_latent = torch.cat([ref_latents, content_ref_latents], dim=1)
        ...
        # print(f"input_condition_latent shape {input_condition_latent.shape}")
        latents[::, :8, :1] = input_condition_latent
        latents[::, 8:] = ref_condition_latent
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                # latents_input = torch.cat([latents, ref_latents, content_ref_latents], dim=1)
                # print(f"latents_input shape is {latents_input.shape}")
                ...
                _, current_timestep = self.add_input_condition(
                        latent_model_input, input_condition_latent, [0], [0], current_timestep
                )
                ...
                # print(f"v_pred shape is {v_pred.shape}")
                # perform guidance
                if do_classifier_free_guidance:
                ...
                latents[::, :8] = latents[::, :8] - dts[i] * v_pred
