...
# # ==================== 动态获取 GPU 数量逻辑 ====================
# # 1. 如果你没有设置 CUDA_VISIBLE_DEVICES，默认给你分配 0 号卡（单卡模式）
# if [ -z "$CUDA_VISIBLE_DEVICES" ]; then
#     echo "未检测到指定显卡，默认使用单卡 (GPU 0)"
#     export CUDA_VISIBLE_DEVICES="0"
# fi

# # 2. 通过逗号的数量，自动计算你要使用的卡数
# # (例如：设为 "2,3,4,5,6,7" 会算出 6; "3" 会算出 1)
# np=$(echo "$CUDA_VISIBLE_DEVICES" | awk -F',' '{print NF}')
# # ===============================================================

# ip_addr="127.0.0.1"
# echo "GPU number: $np (Selected GPUs: $CUDA_VISIBLE_DEVICES)"
...
# # 提取端口号（做了下兼容，防止正则匹配出错）
# Port=$(cat /etc/ssh/ssh_config | grep -i '^Port ' | awk '{print $2}')
# if [ -z "$Port" ]; then
#     Port=22
# fi

# # 注意：这里增加了 -x CUDA_VISIBLE_DEVICES \
# mpirun --allow-run-as-root -np $np \
#     -mca plm_rsh_args "-p ${Port}"  \
#         -hostfile $hostfile \
