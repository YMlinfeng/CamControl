...
from ..models import TextEncoderConfig, Transformer2DModelConfig, UNet2DConfig, VisualTokenizer, VisualTokenizerConfig
...
class BasePipelineConfig(InstantiateConfig):
    ...
    def from_pretrained(self, **kwargs) -> Any:
        ...
        if self.enable_vae_slicing and not "num_frames" in set(inspect.signature(pipeline.vae.forward).parameters.keys()):
...
class PipelineMixin:
    ...
    @torch.no_grad()
    def get_first_frame_latent(self, inputs, is_auto_pack=True):
        latents = []
        samples = inputs
        if not isinstance(samples, list):
            samples = [samples]
        ...
        if len(latents) > 1 and is_auto_pack:
            latents, _ = pack_data(latents)
        elif len(latents) > 1:
            latents, _ = latents, None
        else:
            latents, _ = latents[0], None
        return latents, _
    
    @torch.no_grad()
    def get_latents_for_conditions_nocopy_vae(self, inputs, is_auto_pack=True):  # No Copy Vae
        latents = []
        samples = inputs
        if not isinstance(samples, list):
            samples = [samples]
