...
class RotaryEmbeddingFast(nn.Module):
    def __init__(self, embed_dim, patch_resolution, theta=10000.0):
        ...
        self.axis_embed_dim = embed_dim // self.num_axis
    ...
    def compute_position_embedding(self):
        ...
        if self.num_axis == 1:
            ...
            position = torch.cat((temperal, height, width), dim=-1)
...
class CombinedTimestepConditionEmbeddings(nn.Module):
    ...
    def forward(
        self, timestep, batch_size, hidden_dtype, frames=None, resolution=None, unpadded_resolution=None, frames_resolution=None, prompt_embeds_pooled=None, fps=None, aspect_ratio=None,
    ):
