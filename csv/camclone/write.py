...
def concat_lr(ref_frames, gen_frames):
    """把 ref 和 gen 两段帧序列左右拼接，ref 按 gen 高度缩放对齐"""
    # --- 保护 3: 帧数不一致时对齐到较短的一段 ---
    n = min(len(ref_frames), len(gen_frames))
    if len(ref_frames) != len(gen_frames):
        print(
            f"[warn] 帧数不一致: ref={len(ref_frames)}, gen={len(gen_frames)}, "
            f"截断到 {n} 帧"
        )
    ...
    out = []
    for ref_f, gen_f in zip(ref_frames, gen_frames):
        gen_h = gen_f.shape[0]
        ref_resized = resize_to_height(ref_f, gen_h)
        combined = np.concatenate([ref_resized, gen_f], axis=1)  # 沿宽度方向拼接
        # --- 保护 4: 宽高裁成偶数 ---
        combined = make_even(combined)
        out.append(combined)
...
def main():
    ...
    for i, row in df.iterrows():
        ...
        try:
            ...
            # 参考视频：先按 frame_indexes 取 77 帧，再按 index_new 重采样
            vr_ref = VideoReader(ref_path, ctx=cpu(0))
            ref_77 = vr_ref.get_batch(frame_indexes).asnumpy()
            ref_idx = [min(int(x), ref_77.shape[0] - 1) for x in index_new]
            ref_frames = ref_77[ref_idx]

            # 左右拼接，ref 按 gen 高度缩放对齐
            combined = concat_lr(ref_frames, gen_frames)

            # 保存拼接后的视频
            base_name = os.path.splitext(os.path.basename(gen_path))[0]
            save_path = os.path.join(SAVE_DIR, f"{base_name}_concat.mp4")
            save_video_from_frames(combined, save_path, SAVE_FPS)

            results.append(
                {
                    "ref_video": ref_path,
                    "camclone_gen": gen_path,
                    "concat_video": save_path,
            ...
            print(
                f"[{i}] 保存拼接视频 -> {save_path} ({combined.shape[0]} frames, "
                f"{combined.shape[2]}x{combined.shape[1]})"
            )
            print("done")
